...
class CycleGANSemanticModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        if 'A_label' in input and 'B_label' in input:
            self.input_A_label = torch.from_numpy(np.array(input['A_label' if AtoB else 'B_label'])).to(self.device)
            self.input_B_label = torch.from_numpy(np.array(input['B_label' if AtoB else 'A_label'])).to(self.device)
    ...
    def backward_G(self):
        ...
        #if True:
        if not hasattr(self, 'loss_CLS') or self.loss_CLS.detach().item() > 1.0: # Beer try 0.2 * loss
